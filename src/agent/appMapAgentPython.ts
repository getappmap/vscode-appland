...
import AppMapAgent, { AppMapAgentBase, FilesResponse, InitResponse, InstallResult, StatusResponse } from "./appMapAgent";
...
export default class AppMapAgentPython extends AppMapAgentBase implements AppMapAgent {
    ...
    private async projectExec(path: PathLike, 
        cmd: string, 
    ...
    async install(path: PathLike): Promise<InstallResult> {
        if (this.usesPoetry(path)) {
            ...
            const {stderr, exitCode} = await exec('poetry', cmd.concat([pkg]), { cwd: path as string, output: true });
            if (exitCode !== 0) {
                throw Error(stderr);
            }
            return 'installed';
